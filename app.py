...
def calcular_produccion_vegetales(especie, cantidad_plantas, costo_semilla, precio_venta):
    ...
    tabla_consumo = pd.DataFrame({
        "Mes": list(range(1, int(datos["meses_produccion"]) + 1)),
        "Consumo Agua (litros)": [redondear_cantidad(cantidad_plantas * datos["consumo_agua_mensual"]) for _ in range(int(datos["meses_produccion"]))],
        "Costo Nutrientes (S/)": [formatear_numero(cantidad_plantas * datos["costo_nutrientes"]) for _ in range(int(datos["meses_produccion"]))],
    })
